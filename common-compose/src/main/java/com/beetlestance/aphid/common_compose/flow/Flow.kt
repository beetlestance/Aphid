...
package com.beetlestance.aphid.common_compose.flow
...
private fun Flow(
    ...
    Layout(content) { measurables, outerConstraints ->
        ...
        val childConstraints = if (orientation == Orientation.Horizontal) {
        ...
        fun canAddToCurrentSequence(placeable: Placeable) =
            currentSequence.isEmpty() || currentMainAxisSize + mainAxisSpacing.toIntPx(density) +
                    placeable.mainAxisSize() <= constraints.mainAxisMax

        // Store current sequence information and start a new sequence.
        fun startNewSequence() {
            if (sequences.isNotEmpty()) {
                crossAxisSpace += crossAxisSpacing.toIntPx(density)
            }
        ...
        for (measurable in measurables) {
            ...
            if (currentSequence.isNotEmpty()) {
                currentMainAxisSize += mainAxisSpacing.toIntPx(density)
            }
        ...
        val layoutWidth = if (orientation == Orientation.Horizontal) {
            mainAxisLayoutSize
        } else {
            crossAxisLayoutSize
        }
        val layoutHeight = if (orientation == Orientation.Horizontal) {
            crossAxisLayoutSize
        } else {
            mainAxisLayoutSize
        }
            ...
            sequences.forEachIndexed { i, placeables ->
                val childrenMainAxisSizes = IntArray(placeables.size) { j ->
                    placeables[j].mainAxisSize() +
                            if (j < placeables.lastIndex) mainAxisSpacing.toIntPx(density) else 0
                }
                val arrangement: Arrangement.Vertical = if (i < sequences.lastIndex) {
